...
RSpec.describe AIAgent::Tools::FirecrawlService do
  ...
  before do
    create(:installation_config, name: 'AIAGENT_FIRECRAWL_API_KEY', value: api_key)
  end
  ...
  describe '#initialize' do
    ...
    context 'when API key is missing' do
      before do
        InstallationConfig.find_by(name: 'AIAGENT_FIRECRAWL_API_KEY').destroy
      end
    ...
    context 'when API key is nil' do
      before do
        InstallationConfig.find_by(name: 'AIAGENT_FIRECRAWL_API_KEY').update(value: nil)
      end
    ...
    context 'when API key is empty' do
      before do
        InstallationConfig.find_by(name: 'AIAGENT_FIRECRAWL_API_KEY').update(value: '')
      end
