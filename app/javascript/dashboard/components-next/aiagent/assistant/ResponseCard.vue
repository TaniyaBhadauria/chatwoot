...
const statusAction = computed(() => {
  if (props.status === 'pending') {
    return [
      {
        label: t('AIAGENT.RESPONSES.OPTIONS.APPROVE'),
        value: 'approve',
        action: 'approve',
...
const menuItems = computed(() => [
  ...statusAction.value,
  {
    label: t('AIAGENT.RESPONSES.OPTIONS.EDIT_RESPONSE'),
    value: 'edit',
    action: 'edit',
    icon: 'i-lucide-pencil-line',
  },
  {
    label: t('AIAGENT.RESPONSES.OPTIONS.DELETE_RESPONSE'),
    value: 'delete',
    action: 'delete',
    icon: 'i-lucide-trash',
  },
      ...
      <div class="inline-flex items-center">
        ...
        >
          ...
          >
            ...
            <span class="group-hover:underline">
              {{
                t(`AIAGENT.RESPONSES.DOCUMENTABLE.CONVERSATION`, {
        ...
        >
          <i
            ...
            :title="t('AIAGENT.RESPONSES.STATUS.TITLE')"
          />
          {{ t(`AIAGENT.RESPONSES.STATUS.${status.toUpperCase()}`) }}
        </div>
      </div>
      <div
        class="shrink-0 text-sm text-n-slate-11 line-clamp-1 inline-flex items-center gap-1 ml-3"
